...
function ResultsComponent(props: { results: Results<unknown> | undefined, scrollIntoView: boolean }) {
  ...
  if ((props.results.affectedRows || 0) > 0) {
    return <div ref={resultRef}>{props.results.affectedRows} rows affected</div>;
  }

  if (props.results.fields.length > 0) {
    return <div className="card" ref={resultRef}>
  ...
  return <span ref={resultRef}></span>
}
