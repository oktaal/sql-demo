        ...
        {db ? (
          <PGliteProvider db={db}>
            <NotebookComponent queries={[
              ...
              "-- but how do we group by a property which does not exist?\nSELECT id, text, CASE WHEN text LIKE '%?' THEN 1 ELSE 0 END as is_question\nFROM questions\nGROUP BY (CASE WHEN text LIKE '%?' THEN 1 ELSE 0 END), id\nORDER BY id;",
              "-- but this is very repetitive, this could be done with a sub-query:\nSELECT id, text, is_question\nFROM (SELECT id, text, CASE WHEN text LIKE '%?' THEN 1 ELSE 0 END as is_question\n  FROM questions)\nGROUP BY is_question, id, text\nORDER BY id;",
